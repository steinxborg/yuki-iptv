...
class XTream:
    ...
    def __init__(
        ...
        default_cache_path = str(Path(CACHE_DIR) / "xtream")
    ...
    def authenticate(self):
        ...
        if self.state["authenticated"] is False:
            ...
            #        r = request(url, method="GET", timeout=(4), headers=self.connection_headers)
            #        i = 31
            ...
            try:
                r = request(url, method="GET", timeout=(10), headers=self.connection_headers)
            except requests.exceptions.ConnectionError:
                r = None
    ...
    def _get_request(self, url: str, timeout: Tuple = (2, 15)):
        ...
        while i < 10:
            time.sleep(1)
            try:
                r = request(url, method="GET", timeout=timeout, headers=self.connection_headers)
                i = 20
