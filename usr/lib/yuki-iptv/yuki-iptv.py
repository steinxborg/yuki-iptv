...
from yuki_iptv.gui import YukiGUIClass, move_window_to_center
    ...
    try:
        ...
        def show_playlist_editor():
            if playlist_editor.isVisible():
                ...
                move_window_to_center(win, playlist_editor)
        ...
        YukiGUI.create4(keyseq, ICONS_FOLDER)
        ...
        def epgname_btn_action():
            ...
            esw_input_edit()
            move_window_to_center(win, YukiGUI.epg_select_win)
        ...
        def show_license():
            if not YukiGUI.license_win.isVisible():
                move_window_to_center(win, YukiGUI.license_win)
        ...
        def shortcuts_table_clicked(row1, column1):
            if column1 == 1:  # keybind
                ...
                keyseq.setFocus()
                move_window_to_center(win, YukiGUI.shortcuts_win_2)
        ...
        def show_shortcuts():
            if not YukiGUI.shortcuts_win.isVisible():
                ...
                YukiGUI.shortcuts_table.resizeColumnsToContents()
                # end
                move_window_to_center(win, YukiGUI.shortcuts_win)
        ...
        def show_settings():
            if not YukiGUI.settings_win.isVisible():
                move_window_to_center(win, YukiGUI.settings_win)
        ...
        def show_help():
            if not YukiGUI.help_win.isVisible():
                move_window_to_center(win, YukiGUI.help_win)
        ...
        def show_sort():
            if not YukiGUI.sort_win.isVisible():
                ...
                move_window_to_center(win, YukiGUI.sort_win)
        ...
        def init_mpv_player():
            ...
            logger.info(
                f"Using libmpv {YukiData.player.mpv_version.replace('mpv ', '')}"
            )
        ...
        move_window_to_center(win, YukiGUI.epg_win)
        move_window_to_center(win, YukiGUI.settings_win)
        ...
        def settings_context_menu():
            ...
            if (
                ...
                YukiGUI.group_text.setText("")
                YukiGUI.epgname_lbl.setText(_("Default"))
            move_window_to_center(win, YukiGUI.channels_win)
        ...
        def open_external_player():
            move_window_to_center(win, YukiGUI.ext_win)
        ...
        logger.info(f"Custom libmpv options: {json.dumps(options_custom)}")
        ...
        def show_scheduler():
            if YukiGUI.scheduler_win.isVisible():
                ...
                update_scheduler_programme()
                move_window_to_center(win, YukiGUI.scheduler_win)
        ...
        if YukiData.settings["m3u"] and m3u_exists:
            ...
            move_window_to_center(win, gui_playlists_data.playlists_win)
