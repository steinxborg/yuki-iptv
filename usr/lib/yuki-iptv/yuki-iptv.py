...
from multiprocessing import Manager, Process, Pool, set_start_method
...
from yuki_iptv.args import args
...
from yuki_iptv.mpris import mpris_init, seek_event_callback
from yuki_iptv.stream_info import (
    stream_info,
    open_stream_info,
    clear_stream_info,
    monitor_playback,
...
from yuki_iptv.gui_playlists import show_playlists
...
if __name__ == "__main__":
    # /usr/lib/python3.?/multiprocessing/popen_fork.py:67:
    ...
    try:
        ...
        YukiData.mpv_client_api_version = mpv._mpv_client_api_version()
        YukiData.mpv_api_version = (
            " (API v" + ".".join(map(str, YukiData.mpv_client_api_version)) + ")"
        )
        ...
        def setVideoAspect(va):
            if va == 0:
                va = -1
            # Setting video-aspect-override to -1 is deprecated.
            if va == -1 and YukiData.mpv_client_api_version >= (2, 5):
                va = "no"
        ...
        def doPlay(play_url1, ua_ch=def_user_agent, channel_name_0=""):
            ...
            set_player_settings(channel_name_0)
        ...
        def set_player_settings(j):
            try:
                logger.info("set_player_settings waiting for channel load...")
                try:
                    YukiData.player.wait_until_playing()
                except Exception:
                    logger.warning(traceback.format_exc())
                if j == YukiData.playing_channel:
                    logger.info(f"set_player_settings '{j}'")
                    idle_on_metadata()
                    if (
                    ...
                    logger.info(
                        f"Video aspect: {YukiData.player.video_aspect_override}"
                    )
        ...
        def item_clicked(item, custom_url="", archived=False, is_rewind=False):
            is_ic_ok = True
            try:
            ...
            if is_ic_ok:
                ...
                YukiData.current_prog = current_prog
        ...
        YukiData.item_clicked = item_clicked
        ...
        YukiData.mpv_play_pause = mpv_play_pause
        ...
        YukiData.mpv_fullscreen = mpv_fullscreen
        ...
        YukiData.mpv_volume_set = mpv_volume_set
        ...
        win.listWidget.itemDoubleClicked.connect(item_clicked)

        def enterPressed():
            currentItem = win.listWidget.currentItem()
            if currentItem:
                item_clicked(currentItem)
                    ...
                    try:
                        if YukiData.settings["channellogos"] != 3:
                            if movie_logos_request != YukiData.movie_logos_request_old:
                                ...
                                YukiData.movie_logos_process = Process(
        ...
        def mpv_log_handler(mpv_loglevel, component, message):
            ...
            if "sdl" in component:
                if not YukiData.sdl_warning_shown:
                    YukiData.sdl_warning_shown = True
                    logger.warning("SDL detected, switching to vo=x11")
            ...
            if (
                ...
                if not YukiData.software_warning_shown:
                    YukiData.software_warning_shown = True
                    logger.warning("Switching profile to 'sw-fast' to avoid lagging")
                YukiData.player.profile = "sw-fast"

            if (
                "hwdec" in options
                and options["hwdec"] != "no"
                and (
                    "(nvdec)" in mpv_log_str
                    or "(nvdec-copy)" in mpv_log_str
                    or "(cuda)" in mpv_log_str
                    or "(cuda-copy)" in mpv_log_str
                )
                and not YukiData.hwdec_force_disabled
            ):
                YukiData.hwdec_force_disabled = True
                logger.info(
                    "NVIDIA hardware acceleration is broken (see "
                    "https://codeberg.org/liya/yuki-iptv/issues/121"
                    "), force disabling it"
                )
                YukiData.player.hwdec = "no"
            ...
            if (
                ...
                try:
                    ...
                    if lastfile_file_data[0] in YukiData.array_sorted:
                        ...
                        item_clicked(lastfile_file_data[0])
                        setChannelText(f"  {lastfile_file_data[0]}")
                        try:
        ...
        if "hwdec" in options and options["hwdec"] != "no":
            logger.warning(
                "Hardware acceleration is enabled. "
                "This can cause issues and/or memory leaks. "
                "Disable it if you encounter them."
            )
        ...
        def go_channel(i1):
            ...
            if chk_pass:
                ...
                item_clicked(win.listWidget.currentItem())
        ...
        YukiData.prev_channel = prev_channel
        YukiData.next_channel = next_channel
        ...
        YukiData.state.setStyleSheet("color: black; background-color: " + BCOLOR)
        ...
        YukiData.set_playback_speed = set_playback_speed
        ...
        def on_before_quit():
            ...
            try:
                ...
                pass
            try:
                for sch_record in sch_recordings:
                    do_stop_record(sch_record)
            except Exception:
                pass
            try:
            ...
            try:
                ...
                pass
            try:
                if YukiData.settings["m3u"] and YukiData.comboboxIndex != -1:
                    write_option(
                        "comboboxindex",
                        {
            ...
            try:
                ...
                pass
            try:
                if YukiData.win and not YukiData.first_start:
                    write_option(
                        "window",
                        {
                            "x": YukiData.win.geometry().x(),
                            "y": YukiData.win.geometry().y(),
                            "w": YukiData.win.width(),
                            "h": YukiData.win.height(),
                        },
                    )
                    if YukiData.settings["panelposition"] == 2:
                        write_option(
            ...
            try:
                ...
                pass
            logger.info("Exiting completed")

        app.aboutToQuit.connect(on_before_quit)
