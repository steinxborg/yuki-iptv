...
from yuki_iptv.xtream import (
    convert_xtream_to_m3u,
    parse_series,
    load_xtream,
    EmptyXTreamClass,
)
...
def load_playlist():
    ...
    if YukiData.settings["m3u"]:
        ...
        if YukiData.settings["m3u"].startswith("XTREAM::::::::::::::"):
            ...
            if os.path.isfile(YukiData.settings["m3u"]):
                ...
                try:
                    with open(YukiData.settings["m3u"], encoding="utf8") as m3u_file:
                        m3u = m3u_file.read()
                except Exception:
                    logger.warning("Playlist is not UTF-8 encoding")
                    logger.info("Trying to detect encoding...")
                    try:
                        with open(YukiData.settings["m3u"], "rb") as m3u_file:
    ...
    def doSort(array):
        if YukiData.settings["sort"] == 0:
            return array
        elif YukiData.settings["sort"] == 1:
            return sorted(array)
        elif YukiData.settings["sort"] == 2:
            return sorted(array, reverse=True)
        elif YukiData.settings["sort"] == 3:
            try:
                return sorted(array, key=sort_custom)
            except Exception:
                return array
        return array
