...
class PlaylistEditor(QtWidgets.QMainWindow):
    ...
    def filter_table(self):
        for row1 in range(self.table.rowCount()):
            ...
            if item1:
                if (
                    self.data["groupfilter_edit"].text().lower().strip()
    ...
    def populate_toolbar(self):
        ...
        delete_btn.setIcon(QtGui.QIcon(str(YukiData.icons_folder / "trash.png")))
        delete_btn.setToolTip(_("delete row"))
        delete_btn.clicked.connect(self.delete_row)

        # Add new empty row
        add_btn = QtWidgets.QToolButton()
        add_btn.setIcon(QtGui.QIcon(str(YukiData.icons_folder / "plus.png")))
        add_btn.setToolTip(_("add row"))
        add_btn.clicked.connect(self.add_row)

        # Down
        down_btn = QtWidgets.QToolButton()
        down_btn.setIcon(QtGui.QIcon(str(YukiData.icons_folder / "arrow-down.png")))
        down_btn.clicked.connect(lambda: self.move_row(1))

        # Up
        up_btn = QtWidgets.QToolButton()
        up_btn.setIcon(QtGui.QIcon(str(YukiData.icons_folder / "arrow-up.png")))
