...
# faster than datetime.datetime.strptime
...
def parse_as_xmltv(data, settings):
    ...
    for event, elem in iterparse(data):
        if event == "end":
            if elem.tag == "display-name" and elem.text:
                ret["display_names"].append(elem.text)
            elif elem.tag == "icon" and "src" in elem.attrib and elem.attrib["src"]:
                icon = elem.attrib["src"]
            elif elem.tag == "channel" and "id" in elem.attrib and elem.attrib["id"]:
                first = True
                if elem.attrib["id"] not in ret["ids"]:
                    ret["ids"][elem.attrib["id"]] = set()
                for display_name in ret["display_names"]:
                    ret["ids"][elem.attrib["id"]].add(display_name)
                    ret["names"][display_name.lower().strip()] = elem.attrib["id"]
                    if first:
                        first = False
                        ret["_names"].add(display_name.strip())
                ret["icons"][elem.attrib["id"]] = icon
                ret["display_names"].clear()
                icon = ""
            elif elem.tag == "title" and elem.text:
                title = elem.text
            elif elem.tag == "desc" and elem.text:
                desc = elem.text
            elif elem.tag == "category" and elem.text:
                category = elem.text
                ...
                if (
                    ...
                    if start and start != 0 and stop and stop != 0:
                        ret["epg"][elem.attrib["channel"]].append(
                            {
                                "start": start,
                                "stop": stop,
                                "title": title,
