...
def yuki_mpv_set_speed(spd):
    logger.info(f"Set speed to {spd}")
    YukiMenubarData.player.speed = spd


def yuki_trackset(track, type1):
    YukiMenubarData.yuki_track_set(track, type1)
...
def reload_menubar_shortcuts():
    ...
    sec_i_1 = -1
    for i_1 in YukiMenubarData.secs:
        sec_i_1 += 1
...
def init_menubar(data):
    ...
    for i in ((10, "seconds", 10), (1, "minutes", 60), (10, "minutes", 600)):
        for k in ("-", "+"):
            ...
            sec.triggered.connect(
                partial(yuki_mpv_seek, i[2] * -1 if k == "-" else i[2])
            )
    ...
    YukiMenubarData.spds = []

    for spd in (0.25, 0.5, 0.75, 1.25, 1.5, 1.75):
        spd_action = QtGui.QAction(f"{spd}x", data)
        spd_action.triggered.connect(partial(yuki_mpv_set_speed, spd))
        YukiMenubarData.spds.append(spd_action)
...
def populate_menubar(i, menubar, data, track_list=None, playing_channel=None):
    ...
    for spd_action1 in YukiMenubarData.spds:
    ...
    if _("&Help") == "Сп&равка":
        ...
        # lambda because
        # TypeError: donate_triggered() takes 0 positional arguments but 1 was given
        donate_action.triggered.connect(lambda: donate_triggered())
...
def update_menubar(track_list, playing_channel, m3u):
    ...
    if playing_channel:
        ...
        # logger.info(playing_channel + '::::::::::::::' + m3u)
    ...
    for i in YukiMenubarData.menubars:
        ...
        if track_list and playing_channel:
            ...
            for track in track_list:
                if track["type"] == "video":
                    trk = QtGui.QAction(str(track["id"]), YukiMenubarData.data)
                    if track["id"] == YukiMenubarData.player.vid:
                        trk.setIcon(YukiMenubarData.circle_icon)
                    trk.triggered.connect(partial(yuki_trackset, track["id"], "vid"))
                    YukiMenubarData.menubars[i][0].addAction(trk)
                if track["type"] == "audio":
                    if "lang" in track:
                        trk1 = QtGui.QAction(
                        ...
                        trk1 = QtGui.QAction(str(track["id"]), YukiMenubarData.data)
                    if track["id"] == YukiMenubarData.player.aid:
                        trk1.setIcon(YukiMenubarData.circle_icon)
                    trk1.triggered.connect(partial(yuki_trackset, track["id"], "aid"))
                    YukiMenubarData.menubars[i][1].addAction(trk1)
                if track["type"] == "sub":
                    if "lang" in track:
                        trk2 = QtGui.QAction(
                            "{} ({})".format(track["id"], track["lang"]),
                        ...
                        trk2 = QtGui.QAction(str(track["id"]), YukiMenubarData.data)
                    if track["id"] == YukiMenubarData.player.sid:
                        trk2.setIcon(YukiMenubarData.circle_icon)
                    trk2.triggered.connect(partial(yuki_trackset, track["id"], "sid"))
                    YukiMenubarData.menubars[i][2].addAction(trk2)
        else:
