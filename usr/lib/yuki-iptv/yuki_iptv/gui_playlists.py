...
from yuki_iptv.xdg import LOCAL_DIR, home_folder
from yuki_iptv.misc import MAIN_WINDOW_TITLE, YukiData
...
class Playlists_Widget(QtWidgets.QWidget):
    def __init__(self):
        super().__init__()
        ...
        self.layout_2 = QtWidgets.QGridLayout()
        self.layout_2.setAlignment(QtCore.Qt.AlignmentFlag.AlignLeft)
        self.layout_2.addWidget(self.icon_label, 0, 0)
        self.layout_2.addLayout(self.layout, 0, 1)
        ...
        self.btn_select.setIcon(QtGui.QIcon(str(YukiData.icons_folder / "select.png")))
        ...
        self.btn_edit.setIcon(
            QtGui.QIcon(str(YukiData.icons_folder / "edit_playlist.png"))
        )
        self.btn_edit.setMaximumWidth(32)

        self.btn_delete = QtWidgets.QPushButton()
        self.btn_delete.setToolTip(_("Delete"))
        self.btn_delete.setIcon(QtGui.QIcon(str(YukiData.icons_folder / "trash.png")))
        self.btn_delete.setMaximumWidth(32)

        self.widget_3 = QtWidgets.QWidget()
        self.layout_3 = QtWidgets.QHBoxLayout()
        self.layout_3.setAlignment(QtCore.Qt.AlignmentFlag.AlignRight)
        self.layout_3.addWidget(self.btn_select)
        self.layout_3.addWidget(self.btn_edit)
        self.layout_3.addWidget(self.btn_delete)
        self.widget_3.setLayout(self.layout_3)

        self.layout_main = QtWidgets.QHBoxLayout()
        self.layout_main.setSpacing(0)
        self.layout_main.setContentsMargins(0, 0, 0, 0)
        self.layout_main.addWidget(self.widget_2)
...
class PlaylistsWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        yuki_color = "white" if YukiData.use_dark_icon_theme else "#1b1c1c"
        ...
        self.playlists_add.setIcon(QtGui.QIcon(str(YukiData.icons_folder / "add.png")))
        ...
        #     QtGui.QIcon(str(YukiData.icons_folder / "star.png"))
        # )
        ...
        self.playlists_settings.setIcon(
            QtGui.QIcon(str(YukiData.icons_folder / "settings.png"))
        )
        self.playlists_settings.clicked.connect(
        ...
        yuki_color_2 = "white" if YukiData.use_dark_icon_theme else "#424242"
...
def favourites_plus():
    ...
    YukiData.YukiGUI.epg_offset.setValue(0)
...
class Playlists_Edit(QtWidgets.QMainWindow):
    def __init__(self, overwrite=False, overwrite_name=""):
        ...
        self.path_file.setIcon(QtGui.QIcon(str(YukiData.icons_folder / "file.png")))
        ...
        def epg_edits_add():
            ...
            epg_file.setIcon(QtGui.QIcon(str(YukiData.icons_folder / "file.png")))
...
def xtream_playlists_set_icon(icon):
    for _i in range(Data.playlists_win.playlists_list.count()):
        ...
        internal_name = item.data(QtCore.Qt.ItemDataRole.UserRole)
        if internal_name.startswith("playlist:"):
            internal_name = internal_name.replace("playlist:", "", 1)
            if internal_name in YukiData.playlists_saved:
                if YukiData.playlists_saved[internal_name]["m3u"] in XTream_list.data:
...
@idle_function
def show_xtream_playlists_expiration_set(*args, **kwargs):
    for _i in range(Data.playlists_win.playlists_list.count()):
        item = Data.playlists_win.playlists_list.item(_i)
        internal_name = item.data(QtCore.Qt.ItemDataRole.UserRole)
        if internal_name.startswith("playlist:"):
            internal_name = internal_name.replace("playlist:", "", 1)
            if internal_name in YukiData.playlists_saved:
                path = YukiData.playlists_saved[internal_name]["m3u"]
...
@idle_function
def xtream_playlists_show_loading(*args, **kwargs):
    xtream_playlists_set_icon(YukiData.YukiGUI.loading_icon_small)


@idle_function
def xtream_playlists_hide_loading(*args, **kwargs):
    xtream_playlists_set_icon(YukiData.YukiGUI.tv_icon)


@async_gui_blocking_function
def show_xtream_playlists_expiration(*args, **kwargs):
    try:
            ...
            if xtream_data != XTream_list.data:
                ...
                for xt_name in XTream_list.data:
                    ...
                    if "referer" in xt_name[1] and xt_name[1]["referer"]:
                        headers["Referer"] = xt_name[1]["referer"]
                    if "User-Agent" not in headers:
                        headers["User-Agent"] = YukiData.settings["ua"]
                    if "Referer" not in headers and YukiData.settings["referer"]:
                        headers["Referer"] = YukiData.settings["referer"]
                    if "Referer" in headers and headers["Referer"]:
                        originURL = ""
                        if headers["Referer"] and headers["Referer"].endswith("/"):
                            originURL = headers["Referer"][:-1]
                        if originURL:
                ...
                xtream_playlists_hide_loading()
                show_xtream_playlists_expiration_set()
            YukiData.xtream_list_lock = False
    except Exception:
        show_exception(
            f"Exception in show_xtream_playlists_expiration\n{traceback.format_exc()}"
        )


# XTream expiration end
...
def playlist_selected(sel):
    ...
    if sel.startswith("internal:"):
        ...
        if sel in YukiData.playlists_saved:
            ...
            YukiData.YukiGUI.epg_offset.setValue(epg_offset)
    ...
    if YukiData.playlists_saved:
        ...
        with open(Path(LOCAL_DIR) / "favplaylist.m3u", "r") as favplaylist:
            if favplaylist.read() != "#EXTM3U\n#EXTINF:-1,-\nhttp://255.255.255.255\n":
                ...
                playlist_widget = Playlists_Widget()
...
def show_playlists():
    ...
    from yuki_iptv.gui import moveWindowToCenter

    if not Data.playlists_win.isVisible():
        populate_playlists()
        moveWindowToCenter(Data.playlists_win)
