...
class Playlists_Widget(QtWidgets.QWidget):
    def __init__(self, YukiGUI):
        ...
        self.description_label.setFont(YukiGUI.font_italic)
        self.description_label.setWordWrap(True)

        self.icon_label = QtWidgets.QLabel()
        self.icon_label.setPixmap(YukiGUI.tv_icon)
        ...
        self.btn_select.setIcon(
            QtGui.QIcon(str(Path("yuki_iptv", YukiData.ICONS_FOLDER, "select.png")))
        )
        ...
        self.btn_edit.setIcon(
            QtGui.QIcon(
                str(Path("yuki_iptv", YukiData.ICONS_FOLDER, "edit_playlist.png"))
            )
        )
        ...
        self.btn_delete.setIcon(
            QtGui.QIcon(str(Path("yuki_iptv", YukiData.ICONS_FOLDER, "trash.png")))
        )
...
class PlaylistsWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.playlists_add.setIcon(
            QtGui.QIcon(str(Path("yuki_iptv", YukiData.ICONS_FOLDER, "add.png")))
        )
        ...
        #     QtGui.QIcon(str(Path("yuki_iptv", YukiData.ICONS_FOLDER, "star.png")))
        # )
        ...
        self.playlists_settings.setIcon(
            QtGui.QIcon(str(Path("yuki_iptv", YukiData.ICONS_FOLDER, "settings.png")))
        )
...
class Playlists_Edit(QtWidgets.QMainWindow):
    def __init__(self, overwrite=False, overwrite_name=""):
        ...
        self.path_file.setIcon(
            QtGui.QIcon(str(Path("yuki_iptv", YukiData.ICONS_FOLDER, "file.png")))
        )
...
@idle_function
def show_xtream_playlists_expiration_set(unused=None):
    for _i in range(Data.playlists_win.playlists_list.count()):
        item = Data.playlists_win.playlists_list.item(_i)
        _internal_name = item.data(QtCore.Qt.ItemDataRole.UserRole)
        if _internal_name.startswith("playlist:"):
...
@idle_function
def xtream_playlists_show_loading(unused=None):
    xtream_playlists_set_icon(YukiData.YukiGUI.loading_icon_small)


@idle_function
def xtream_playlists_hide_loading(unused=None):
    ...
    if YukiData.playlists_saved:
        ...
        with open(Path(LOCAL_DIR, "favplaylist.m3u"), "r") as favplaylist:
            if favplaylist.read() != "#EXTM3U\n#EXTINF:-1,-\nhttp://255.255.255.255\n":
                ...
                playlist_widget = Playlists_Widget(YukiData.YukiGUI)
...
def show_playlists():
    if not Data.playlists_win.isVisible():
        populate_playlists()
        moveWindowToCenter(YukiData.win, Data.playlists_win)
