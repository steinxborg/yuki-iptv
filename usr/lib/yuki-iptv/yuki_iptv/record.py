...
from yuki_iptv.misc import YukiData, YTDL_NAME
from yuki_iptv.threads import kill_process_childs
...
if not shutil.which("ffmpeg"):
    show_exception("ffmpeg is required")
    sys.exit(1)
...
def exit_handler(process, exit_code, exit_status):
    ...
    if not is_ok or exit_status != QtCore.QProcess.ExitStatus.NormalExit:
        ...
        if not is_killed and not (
            (YTDL_NAME in ffmpeg_proc_program) and exit_code == int(signal.SIGTERM)
        ):
...
def record(
    ...
    cenc_decryption_key_array = []
    if cenc_decryption_key:
        logger.info("DRM is enabled")
        cenc_decryption_key_array = ["-cenc_decryption_key", cenc_decryption_key]
    ...
    if not is_return:
        ...
        YukiData.ffmpeg_proc.start(process, array)
        ...
        ffmpeg_ret_proc.start(process, array)
