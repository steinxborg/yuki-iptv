...
from yuki_iptv.requests_timeout import requests_get, requests_post
...
def extract_cenc_decryption_key_from_url(url):
    ...
    try:
        if "$OPT:" in url:
            ...
            url = url.split("$OPT:")[0]
            for opt in opts:
                if "decryption_key=" in opt:
                    key_match = re.findall(
                        r"decryption_key=([0-9a-zA-Z]{32})", opt
                    )  # KEY_LEN
                    if key_match:
                        key = key_match[0]
    except Exception:
...
def convert_key(key, url, useragent, referer):
    if key.startswith("__LICENSE_URL_"):
        if threading.current_thread() == threading.main_thread():
            executeInMainThread(
                partial(
                    show_exception,
                    (
                        "drm.convert_key is running is main thread. "
        ...
        req = requests_get(url, headers=headers, stream=True, timeout=(35, 35))
        ...
        if not license_url:
            ...
            _key = parse_keys(json.loads(req.content.decode("utf-8")))
            if _key:
                key = _key
            else:
